...
if ! grep -q "Include conf/extra/episciences-manager.conf" /usr/local/apache2/conf/httpd.conf; then
  echo "Include conf/extra/episciences-manager.conf" >> /usr/local/apache2/conf/httpd.conf
fi
